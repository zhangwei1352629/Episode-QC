...
质检提交先保存不可变候选结果，Flow 确认完成并回读成功后才更新数据目录的当前 `qc_result.json`。平台拒绝或响应丢失时保留原当前结果；NAS 镜像更新失败时保留待同步状态，由自动巡检重试同一个结果 ID。仅作抽检、不影响当前结论的任务不会覆盖当前结果。
...
平台每验证完成一个 Episode，仅索引该 Episode，不重扫已下载目录、不重新导入旧标注导出，也不会将尚未下载的其他条目标记为丢失。索引解析不占用标注保存的全局锁，各 Episode 的数据库写入单独提交，已有标注和质检结论保持不变。

当前条优先准备主视角；后台围绕当前条及同任务顺序中的后两条预加载，待执行队列最多三个项目。切换条目会替换旧队列，已开始的转换允许安全完成，不强杀写文件过程。同一条的前台和后台准备互斥，避免重复生成。主视角可播放后，后台再补当前条的完整播放内容。

页面分别显示原文件下载数量、播放就绪数量和播放准备中数量。“原文件已下载”不代表所有视角已准备好；状态“主视角可播放”与“可播放”分开显示。

#### 边缓存边标注（第二阶段）

Web 服务默认采用 spawn 独立进程分别处理索引和播放准备，每类最多一个工作进程；进程降低 CPU 调度优先级，不继承 Web 的连接与锁。数据库、任务调度、Flow 会话和下载仍由原服务管理。保留第一阶段的有界预加载队列与同条目互斥；正在执行的工作不会因切换条目被强行终止。子进程故障会显式报错，不在主进程隐式重做写操作。嵌入式 `EpisodeQcWebApplication` 默认不启用进程隔离，生产入口 `create_web_server` 默认启用。

下载采用共享预算：收到 Episode 播放、详情或标注请求后的 30 秒内，目标上限为 16 MiB/s；空闲时恢复到 64 MiB/s。额度最多积累 1 MiB，保留原有分块大小与状态写入频率；磁盘和网络较慢时实际速度可能更低。此机制依据交互活动调整，并非 CPU/磁盘负载自动控制，哈希校验和播放生成不受下载字节预算约束。

同一次续传内，首次完整哈希校验的结果只在文件大小、修改时间、变更时间和文件身份均未变化时复用，避免第二遍重复校验。校验凭证只保留在本次调用内，重启后仍完整校验，不跨会话信任旧状态。现有标注与质检结论不变。

验收需要在 Windows 实际部署后同时验证下载推进、当前及后续条目播放、标注保存和 Flow 刷新；本地子进程回归不能替代真实工作站上的延迟测量。

#### AI 独立轮次与人工复检

成功的 AI 运行以不可变结果与运行 ID 作为独立 `ai_preannotation` 历史轮，由 Flow 的 `review_history` 下发；它不生成“人工质检合格”事实，也不自动完成 QC Job。仅继承当前批次及已有历史批次对应 Episode 的有效 AI 结果，原文件 SHA-256 不匹配或结果内容摘要异常时不继承。

QC 打开 AI 面板时先验证本地原文件，再沿用多轮质检的继承逻辑创建人工轮的可编辑副本，时间轴标记“AI 预标注 · 待人工复检”。AI 原始结果留在 Flow，人工修改、删除、补标只改变当前副本；后续批次按同一标注血缘继承人工修改与删除，不重复导入或恢复已删除标注。没有 AI 检出项仍需人工作出结论。已完成人工质检的条目不会因 AI 结果晚到而静默增加标注，应在新复检批次处理。

Flow 与 QC 都需升级此协议。未升级的 Flow 仍回退到旧候选确认面板。本地文件校验凭证只在当前服务会话中按文件身份和时间戳复用，文件变化后重新校验。首次校验由独立索引进程执行，不占标注写锁。

Web 普通读取、播放位置和设置保存默认 20 秒超时，播放准备 90 秒，AI 查询和 Flow 登录 120 秒；超时覆盖完整响应体。切换 Episode 会取消旧的只读/播放准备请求并重置本轮画面请求状态，不取消标注或质检保存请求，不停止服务端已经开始的缓存生成。保存请求等待超时时提示结果待核对，不宣称保存失败或自动重试。旧响应不得覆盖新条目的画面。

### 标注终点与 Flow 时长不一致

平台任务按对应 Episode 的 Flow 登记时长与本地媒体时长中较短者限制标注。原始媒体的播放时间轴不变，页面单独显示“可标注范围”；I/O、拖动选区、时间点、整条标签和编辑保存都遵守该范围。编辑器保留纳秒精度，避免重新保存时因毫秒四舍五入产生越界。

打开平台任务时会刷新本地的 Flow 时长快照；有效快照存在时可离线标注。缺少有效时长或 Episode 对应不唯一时会禁止新增、编辑和确认完成，请联网重新打开任务同步。不会猜测时长、移动时间起点或扩大 Flow 登记时长。

现有历史标注不会被后台批量截断。确认完成时会检查本条全部标注，提交前会汇总批次中的时长越界项；请核对并在编辑器中修正，再提交。已提交的历史结果及原始媒体保持不变。
