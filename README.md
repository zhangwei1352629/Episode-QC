...
Web 模式下，“添加数据目录”支持服务所在电脑上的绝对路径，也支持已登录挂载的
`smb://服务器/共享/目录` NAS 地址。“导入标签库”和“导出结果”仍使用本机路径。
MCAP 始终由 Python 后端直接读取，不会上传到浏览器。
...
1. 单击右上角“添加数据目录”；
2. Web 模式输入包含一个或多个 `episode_*` 的绝对路径，或输入
   `smb://服务器/共享/目录`；Electron 模式可在目录选择器中打开已经登录的 NAS；
