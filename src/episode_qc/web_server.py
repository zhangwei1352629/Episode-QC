...
def default_web_paths(workspace_root: str | Path | None = None) -> WebPaths:
    ...
    return WebPaths(
        ...
        default_label_schema=(
            packaged_defaults / "label_schema_v1.example.yaml"
            if packaged_defaults.is_dir()
            else project_root / "mocap_qc_v1_design_bundle" / "label_schema_v1.example.yaml"
        ),
    )
...
class EpisodeQcWebApplication:
    def __init__(self, paths: WebPaths, *, token: str | None = None) -> None:
        self.paths = paths
    ...
    def get_workspace_state(self) -> dict[str, object]:
        state = workspace_state(self.paths.db_path)
...
class EpisodeQcWebServer(ThreadingHTTPServer):
    ...
    def __init__(self, server_address: tuple[str, int], application: EpisodeQcWebApplication) -> None:
...
class EpisodeQcRequestHandler(BaseHTTPRequestHandler):
    ...
    def _assert_api_access(self, parsed: Any) -> None:
        ...
        if host not in {"127.0.0.1", "localhost", "::1"}:
            raise PermissionError("拒绝非本机 Host")
    ...
    def _route_api(self, method: str, path: str, query: dict[str, list[str]]) -> None:
        ...
        if method == "GET" and path == "/api/workspace":
            self._send_json(app.get_workspace_state())
            return
...
def create_web_server(
    ...
    *,
    ...
    application = EpisodeQcWebApplication(paths, token=token)
    return EpisodeQcWebServer(("127.0.0.1", port), application)
...
def serve_web_app(
    *,
    ...
    server = create_web_server(default_web_paths(workspace_root), port=port)
    actual_port = server.server_address[1]
    url = f"http://127.0.0.1:{actual_port}/?token={server.application.token}"
    print(f"Episode QC Web: {url}", flush=True)
    if open_browser:
