...
后台缓存不会驱逐正在下载、部分就绪、失败可恢复或未同步结果的原始 Episode 缓存。2026-08-13 起，Web 服务只会按统一的 24 小时留存策略回收已完整同步的 `ready` Job；已验证的缓存、工作区标注和质检进度在不满足该条件时继续保留，显式 `platform-evict` 仍可用于人工运维。
...
Flow 平台缓存的定时过期清理在 Web 服务启动后恢复；`cache_job()` 会在每个 Episode 的磁盘检查前调用同一安全策略。该策略只删除已同步且超过 24 小时的 `ready` Job，绝不在磁盘压力下删除未同步或未到期缓存。
